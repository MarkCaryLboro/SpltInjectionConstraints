classdef multiShot < singleShot 
    ...
    methods
        function obj = multiShot( CalStructure, DI_IPW_SEP_IDK, NumIntShots )
            %--------------------------------------------------------------------------
            % twoShots class constructor function
            %
            % obj = multiShot( CalStructure, DI_IPW_SEP_IDK, NumIntShots );
            % 
            % Input Arguments:
            %
            ...
            %
            % DI_IPW_SEP_IDK    --> Minimum  {eparation between intake
            %                       injections
            % NumIntShots       --> Desired number of intake shots { 2 }
            %--------------------------------------------------------------------------
            obj = obj@singleShot( CalStructure );
            obj.DI_IPW_SEP_IDK = DI_IPW_SEP_IDK;
            if ( nargin < 3 ) || isempty( NumIntShots )
                obj.NumIntShots = "DoubleShot";
            else
                obj.NumIntShots = splitInjectionCounts( NumIntShots );
            end
        end
